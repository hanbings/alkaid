...
public interface SyncQueryActions {
     ...
     * @return 数据实体列表
     */
    <T> List<T> read(Map<String, Object> index, Class<T> type);
     ...
     * 伪代码: <br>
     * search(String data, int top, int bottom, int limit, Class<T> type)
     *
     * @param data   字段名称
     * @param top    边界
     * @param bottom 边界
     ...
     * @return 数据实体列表
     */
    <T, V> List<T> search(String data, V top, V bottom, int limit, Class<T> type);
}
